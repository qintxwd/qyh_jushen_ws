...
#endif
    PM_RPY(double _y, double _p, double _r);
...
/* m * m */
extern PM_ROTATION_MATRIX operator*(const PM_ROTATION_MATRIX& m1, const PM_ROTATION_MATRIX& m2);
...
typedef struct
{
    PmCartesian start;
    PmCartesian end;
    PmCartesian uVec;                 // unit direction vector
    PmCartesian uVec_blend_for_cur;   //该运动段做转接时使用的方向向量
    PmCartesian uVec_blend_for_prev;  //前一运动段做转接时使用的方向向量
    double tmag;
    int tmag_zero;
} PmCartLine;
...
extern int pmRotAroundX(double theta, PmRotationMatrix* rotMatrix);
extern int pmRotAroundY(double theta, PmRotationMatrix* rotMatrix);
extern int pmRotAroundZ(double theta, PmRotationMatrix* rotMatrix);
extern int pmRotArbitAxisWithAngleConvert(PmCartesian const* const waypoint, PmCartesian* normDir, double angle, PmHomogeneous* resHom);
...
extern int pmCheckCartZero(PmCartesian* const v);
/*! \todo Another #if 0 */
#if 0
...
extern int pmCartPlaneProj(PmCartesian const* const v, PmCartesian const* const normal, PmCartesian* vout);
...
extern int pmQuatAxisAngleMult(PmQuaternion const* const, PmAxis, double, PmQuaternion*);
...
extern int pmMatCartMult(PmRotationMatrix const* const m, PmCartesian const* const v, PmCartesian* const vout);
extern int pmMatMatMult(PmRotationMatrix const* const m1, PmRotationMatrix const* const m2, PmRotationMatrix* const mout);
extern int pmRpyRpyAdd(PmRpy const* const iniRpy, PmRpy const* const deltRpy, PmRpy* const res);
extern int pmRpyRpySub(PmRpy const* const iniRpy, PmRpy const* const deltRpy, PmRpy* const res);
extern int pmMatXyzConvert(PmRotationMatrix const* const m1, PmEulerXyz* const Xyz);
extern int pmRxRegMatConvert(double const* const RxReg, PmRotationMatrix* const m1);  //Rotation matrix rotating around X axis (radian system)
extern int pmRzRegMatConvert(double const* const RzReg, PmRotationMatrix* const m1);  //Rotation matrix rotating around Z axis (radian system)
...
extern int pmHomHomMult(PmHomogeneous const* const, PmHomogeneous const* const, PmHomogeneous* const);
...
/*deal with the RPY angle in the critical pose*/
extern int pmRpyPostProcess(PmCartLine* const line);
...
extern double fsign(double f);

int calculate_angle_between_vectors(PmCartesian const* const vec_1, PmCartesian const* const vec_2, double* const angle);
/* slicky macros for item-by-item copying between C and C++ structs */

#define toCart(src, dst)    \
    {                       \
        (dst)->x = (src).x; \
        (dst)->y = (src).y; \
        (dst)->z = (src).z; \
