...
private:
  ...
  void handleWaistWithAxis(double lx)
  {
    ...
    if (lx < -deadzone) {
      ...
      desired_angle = waist_current_angle_; // stop by commanding current angle
    }

    // only send if changed (avoid spamming)
    const double eps = 0.01;
    if (std::abs(desired_angle - last_sent_waist_angle_) > eps) {
      sendWaistCommand(5 /*CMD_GO_ANGLE*/, static_cast<float>(desired_angle), false);
      last_sent_waist_angle_ = desired_angle;
      waist_leanning_ = (desired_angle > 0.5);
    }
  }
