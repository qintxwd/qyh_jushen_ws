#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Right-hand teleop node (mirrored from left). Uses `base_link_right` / `rt` frames from URDF.
import rclpy
...
class QyhTeleopNode(Node):
    def __init__(self):
        super().__init__('qyh_teleop_right')

        # publishers
        self.right_servo_pub = self.create_publisher(JointState, '/teleop/right/servo_p', 10)

        # subscribers
        self.right_pose_sub = self.create_subscription(PoseStamped, '/vr/right_controller/pose', self.vr_right_pose_callback, 10)
        self.right_joy_sub = self.create_subscription(Joy, '/vr/right_controller/joy', self.right_joy_callback, 10)
        self.right_tcp_sub = self.create_subscription(JointState, '/right_arm/tcp_pose', self.right_tcp_pose_callback, 10)
        ...
        # state machine
        self.right_clutch_state = 'IDLE'
        # current values
        self.right_grip_value_ = 0.0
        self.cur_tcp_position = [0.0, 0.0, 0.0]
        self.cur_tcp_rotm = np.eye(3)
        self.cur_tcp_rpy = [0.0, 0.0, 0.0]
        self.tcp_ready = False

        # base_link -> base_link_right (use values from URDF joint base_to_right)
        # URDF: <joint name="base_to_right" origin xyz="-0.001080 -0.083950 0.000951" rpy="0 0 0.5236" />
        self.T_base_base_right = self.make_transform_matrix(
            translation=[-0.001080, -0.083950, 0.000951],
            rpy=[0, 0, 0.5236]
        )

        # rt -> forward_rt (match left forward frame rotation)
        self.T_rt_forward = self.make_transform_matrix(
            translation=[0, 0, 0],
    ...
    def right_joy_callback(self, msg):
        if hasattr(msg, 'axes') and len(msg.axes) > 3:
            self.right_grip_value_ = float(msg.axes[3])

    def right_tcp_pose_callback(self, msg):
        mm_to_m = 0.001
        try:
    ...
    def vr_right_pose_callback(self, msg):
        grip = getattr(self, 'right_grip_value_', 0.0)
        ...
        state = self.right_clutch_state

        if state == 'IDLE':
            if pressed:
                self.right_clutch_state = 'ENGAGING'
            return

        if state == 'ENGAGING':
            try:
                T_vr_in_right = self.invert_transform(self.T_base_base_right) @ T_vr_now_base
                self.vr_start_rotm = T_vr_in_right[:3, :3].copy()
                self.vr_start_position = T_vr_in_right[:3, 3].copy()
                self.arm_start_rpy = np.array(self.cur_tcp_rpy).copy()
                self.arm_start_position = np.array(self.cur_tcp_position).copy()

                self.right_clutch_state = 'TRACKING'
                ...
                self.get_logger().error(f"Error in ENGAGING state (right): {e}")
                self.right_clutch_state = 'IDLE'
                return
        ...
        if state == 'TRACKING':
            if released:
                self.right_clutch_state = 'IDLE'
                self.get_logger().info('[RIGHT] Clutch RELEASED')
                return

        if self.right_clutch_state != 'TRACKING':
            return
        ...
        try:
            T_vr_current_in_right = self.invert_transform(self.T_base_base_right) @ T_vr_now_base
            vr_current_rotm = T_vr_current_in_right[:3, :3]
            vr_current_pos = T_vr_current_in_right[:3, 3]
            delta_rotm_vr = vr_current_rotm @ np.linalg.inv(self.vr_start_rotm)
            delta_rotm_rt = delta_rotm_vr
            arm_start_rotm = R.from_euler('xyz', self.arm_start_rpy).as_matrix()
            target_rotm = delta_rotm_rt @ arm_start_rotm
            ...
            self.get_logger().error(f"Error in rotation matrix calculation (right): {e}")
            import traceback
    ...
    def publish_servo_p_command(self, pose):
        ...
        self.right_servo_pub.publish(msg)
