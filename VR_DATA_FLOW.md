...
│       ▼ Clutch 状态机                                           │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  状态: IDLE → ENGAGING → TRACKING → RELEASING → IDLE    │   │
│  │                                                          │   │
│  │  • IDLE: grip < 0.2, 机械臂保持位置                      │   │
│  │  • ENGAGING: grip > 0.8, 建立 VR-机器人 参考映射         │   │
│  │  • TRACKING: 跟踪 VR 相对增量                            │   │
│  │  • RELEASING: grip < 0.2, 保持最后位置                   │   │
│  └─────────────────────────────────────────────────────────┘   │
│       │                                                         │
│       ▼ 增量计算                                                │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  delta_pos = (vr_current - vr_reference) * scale        │   │
│  │  target_pos = robot_reference + delta_pos               │   │
│  │                                                          │   │
│  │  限制: max_position_delta = 0.05m/step                  │   │
│  │        max_rotation_delta = 0.1rad/step                 │   │
│  └─────────────────────────────────────────────────────────┘   │
│       │                                                         │
│       ▼ 发布目标位姿                                            │
│          /sim/left_target_pose, /sim/right_target_pose         │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  订阅: /sim/left_target_pose, /sim/right_target_pose           │
│        (来自 vr_clutch_node 的目标位姿)                         │
│       │                                                         │
│       ▼ 坐标变换 (VR坐标系 -> 机器人基坐标系)                    │
│       │                                                         │
...
│       ▼ 映射规则                                                │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  • Trigger (axes[2]) 连续值映射到夹爪开度                 │   │
│  │    - Trigger = 0.0  →  夹爪完全张开                       │   │
│  │    - Trigger = 1.0  →  夹爪完全闭合                       │   │
│  │                                                           │   │
│  │  ⚠️ 注意: Grip (axes[3]) 用于 Clutch 离合器控制          │   │
│  │    - Grip > 0.8: 机械臂跟随 VR 手柄移动                   │   │
│  │    - Grip < 0.2: 机械臂保持位置不动                       │   │
│  └─────────────────────────────────────────────────────────┘   │
│       │                                                         │
│       ▼ qyh_gripper_control                                    │
